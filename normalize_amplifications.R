### Script used to normalize the expression data corresponding to one single gene for all the samples from the same TCGA project. DESeq2 method is used to normalize and afterwards compare between the groups of wild-type vs mutated. This script is used for amplifications.
...
# f_patients <- "/workspace/projects/cndrivers/tcga/tcga_acc/gistic_output/all_thresholded.by_genes.txt"
f_patients <- commandArgs(trailingOnly = TRUE)[3]
# f_expression <- "/workspace/projects/cndrivers/tcga/tcga_acc/acc_expression_mtx.tsv"
f_expression <- commandArgs(trailingOnly = TRUE)[4]
# new_dict <- "/workspace/projects/cndrivers/tcga/tcga_acc/tcga_acc_2colsdict.tsv"
new_dict <- commandArgs(trailingOnly = TRUE)[5]
# out_file <- "/workspace/projects/cndrivers/tcga/tcga_acc/tmp_deseq2_amps.tsv"
