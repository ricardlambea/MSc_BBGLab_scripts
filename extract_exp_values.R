### This script has to be executed for each one of the projects, and then for each project we'll have one file with all the genes and their expression counts.

# c_gene <- "APOD"
c_gene <- commandArgs(trailingOnly = TRUE)[1] # take first argument after script's name
print(paste("Chosen gene is", c_gene, sep=" "))
# gene2ensembl <-  "/workspace/projects/cndrivers/tcga/ensg_symbol.tsv"
gene2ensembl <-  commandArgs(trailingOnly = TRUE)[2]
# f_patients <- "/workspace/projects/cndrivers/tcga/tcga_cesc/gistic_output/all_thresholded.by_genes.txt"
f_patients <- commandArgs(trailingOnly = TRUE)[3]
# f_expression <- "/workspace/projects/cndrivers/tcga/tcga_cesc/cesc_expression_mtx.tsv"
f_expression <- commandArgs(trailingOnly = TRUE)[4]
# new_dict <- "/workspace/projects/cndrivers/tcga/tcga_cesc/tcga_cesc_2colsdict.tsv"
new_dict <- commandArgs(trailingOnly = TRUE)[5]
# out_file <- "/workspace/projects/cndrivers/test_cesc.tsv"
out_file <- commandArgs(trailingOnly = TRUE)[6]

gene2ensembl <- read.table(gene2ensembl, header = TRUE, stringsAsFactors = FALSE, sep = "\t") 
...
c_patients <- colnames(all_patients)[all_patients[c_gene,] > -1] # get rid of deletions
...
levels(colData$cases) <- c("FALSE" = "AMPLIFIED", "TRUE" = "DIPLOID")
...
gplot <- ggplot(data = gene_plot) + 
  ...
  ggtitle(paste("Boxplot of gene", c_gene, ".\nProject TCGA-BRCA amplifications", sep = " ")) + xlab("Cases") + ylab("Expression normalized counts") + labs(fill = "Cases")
 #gplot

c_gene_res$symbol <- c_gene
gene_plot$Gene <- c_gene

write.table(gene_plot, col.names = FALSE, append = TRUE, row.names = TRUE, file = out_file, sep="\t")
# header = sample	count	cases	gene  # headers used in write.table, but they are not printed into the generated file
